    ...
    : QWidget(parent)
{
    clearqipan();
    qir=15;  //设置棋子的大小
    setMinimumSize(qir*20*2+20,qir*20*2+20);//设置窗口大小
    startset(0);  //黑子先下棋，-1代表为空，0代表黑子，1代表白子
...
void  MainWindow::startset(int _chuqi1){   //开始建立
    chuzi1=_chuqi1;   //设置先下一方
    chuzi=_chuqi1;   //设置当前一方
    update();   //更新,绘制棋盘
}
void MainWindow::chujie(int hang,  int lie){  //出界判定
    if(hang<1||hang>20||lie<1||lie>20)   return;
    if(a[hang][lie]!=-1)  return;
    a[hang][lie]=chuzi;
}
void MainWindow::clearqipan(){
    for(int i=0;i<21;i++){
        for(int j=1;j<21;j++)
            a[i][j]=-1;
    }
...
void MainWindow::painqizi(QPainter &q){ //绘制棋子
    //QColor ys;
    for(int i=1;i<21;i++){  //黑子
        for(int j=1;j<21;j++){
            if(a[i][j]==0){
               q.setBrush(Qt::black);
               q.setPen(QPen(QBrush(Qt::black), 2));
    ...
    for(int i=1;i<21;i++){  //白子
        for(int j=1;j<21;j++){
            if(a[i][j]==0){
               q.setBrush(Qt::white);
               q.setPen(QPen(QBrush(Qt::white), 2));
               q.drawEllipse(qizi(i,j));
            }
        }
    }
}
QPoint MainWindow::zhongxin(int hang,int lie){   //棋子的中心坐标
    QPoint q1(qir*(lie-1)*2,qir*(hang-1)*2);
    return q1+dian;
}
QPoint MainWindow::zuoshang(int hang,int lie){//棋子左上角到中心的距离，即左上角的大小
    return zhongxin(hang,lie)-QPoint(qir,qir);
}
QRect MainWindow::qizi(int hang,int lie){  //棋子所在的矩形
    return QRect(zuoshang(hang,lie),QSize(qir*2-1,qir*2-1));
}
bool MainWindow::dianzaiyuan(QPoint q1,int &hang,int &lie){  //判断像素点是否在棋子内
...
bool MainWindow::judge(int hang, int lie){  //判断胜负
